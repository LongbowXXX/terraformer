...
### 2. Prohibit "Vibe Coding"

"Vibe Coding" refers to the dangerous practice of blindly hitting `Tab` to accept Copilot suggestions because they "look right" (good vibes) without verifying the logic.

- **🚨 Strict Prohibition**: Reviewing the code _after_ generation is mandatory. Vibe coding leads to technical debt and security holes.

### 3. Slopsquatting (Package Hallucination)

AI often "hallucinates" package names. It might combine keywords to invent a library that _sounds_ real (e.g., `react-native-crypto-secure-storage`).

- **The Attack**: Malicious actors register these hallucinated names on npm/PyPI to distribute malware. This is called "Slopsquatting".
- **🚨 MANDATORY RULE**: **Never install a package suggested by AI without verifying its existence and authenticity on the official registry (npm, PyPI, etc.) first.**
